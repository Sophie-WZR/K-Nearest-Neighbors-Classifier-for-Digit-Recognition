...
public class MNIST {
    ...
    public static class DataHolder implements Comparable<DataHolder> {
        ...
        /**
         * * Compares this DataHolder object with another to determine ordering based on priority.
         *
     ...
     * @throws IllegalArgumentException if the input arrays have different lengths, indicating
    ...
    public static DataHolder[] getClosestMatches(float[] image, int k) {
        DataHolder[] kClosest = new DataHolder[k];
        MyPriorityQueue<DataHolder> distance = new MyPriorityQueue(TRAIN_IMAGES.length);
        for (int i = 0; i < TRAIN_IMAGES.length; i++){
            DataHolder singleDis = new DataHolder(TRAIN_LABELS[i],
                    totalDist(TRAIN_IMAGES[i], image), TRAIN_IMAGES[i]);
            distance.offer(singleDis);
        }
        for (int i = 0; i < k; i++){
            kClosest[i] = (DataHolder) distance.poll();
        }
        return kClosest;
    }
     ...
     * @return the most common label as an integer between 0 and 9, which represents the recognized
     * number of the image. In case of a tie, the smallest numerical label is returned.
     *
     */
    public static int predict(DataHolder[] closestMatches) {
        ...
        for (int i=0; i<counts.length; i++){
            if(counts[i]>max || (counts[i]==max && commonLabel>i)){
                max = counts[i];
                commonLabel = i;
            }
        }
